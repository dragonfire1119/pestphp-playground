...
trait RefreshDatabase
{
    ...
    {
        if (!$this->refreshDatabaseState()) {
            $this->artisan('wipe:db:all');

            foreach ($this->connectionsToTransact() as $app) {
                $this->artisan('migrate:db', ['--name' => $app]);
            }

            $this->app[Kernel::class]->setArtisan(null);

            $this->setRefreshDatabaseState(true);
        } else {
            // ray('Did not migrated db')->orange();
        }
